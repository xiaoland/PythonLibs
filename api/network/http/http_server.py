...
from api.network.http.http_handler import HttpHandler
...
def run_server(ba):
    ...
    # log.add_log("http server listening on %s:%s" % (setting["bindIp"], str(setting["httpPort"])), 1)
    # flask_app.run(host=setting["bindIp"], port=setting["httpPort"])
    flask_app.run("0.0.0.0", 5000)


@flask_app.route('/api/<path:func>', methods=["POST", "GET", "PUT", "PATCH", "DELETE"])
